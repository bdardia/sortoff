...
void sort_step(HDC hdc, uint bitoffset, concurrency::array<uint> & src, concurrency::array<uint> & dest,
...
namespace pal
{
	void radix_sort(HDC hdc, concurrency::array<uint>& arr)
	{
		...
		for (uint i = 0; i < 16; i++)
		{
			...
			sort_step(hdc, bitoffset, src, dest, interm_prefix_sums, num_tiles);
		}
	}
